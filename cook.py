...
def cook_session(session: Session):
    os.system(f"tmux new-session -d -s {session.name} -n {session.windows[0].name} -c {session.root}")
...
def cook_window(window: Window, session: Session, needs_creation: bool=True):
    ...
    for i, pane in enumerate(window.panes):
        ...
        for command in commands:
            os.system(f"tmux send-keys -t {session.name}:{window.name}.{i} '{command}' ENTER")
